...
import { createDocument, createObject, createPage, getActivePage, persistDocument, restoreDocument, type BoardDocument, type CoreObjectType, type CoreObject } from "@/lib/coreBoard";
...
export default function TeacherProductShell() {
  ...
  const selectedObject = activeObjects.find((item) => item.id === selectedObjectId) ?? activeObjects[activeObjects.length - 1] ?? null;
  ...
  function addContent(type: CoreObjectType) { const item = createObject(type, objectSeed[type], 54 + (activeObjects.length % 3) * 190, 52 + Math.floor(activeObjects.length / 3) * 100); const pages = board.pages.map((page) => page.id === board.activePageId ? { ...page, objects: [...page.objects, item] } : page); setSelectedObjectId(item.id); updateBoard({ ...board, pages }, `${objectLabels[type]} أضيفت إلى الصفحة ${pageNumber}.`); }
  function addContextualActivity(object: CoreObject) { const pages = board.pages.map((page) => page.id === board.activePageId ? { ...page, objects: [...page.objects, object] } : page); setSelectedObjectId(object.id); updateBoard({ ...board, pages }, "أضيف نشاط سياقي جديد مع رابط المصدر."); }
