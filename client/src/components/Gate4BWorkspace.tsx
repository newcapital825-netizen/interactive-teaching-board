...
function FeedbackCard({ journey }: { journey: JourneyState }) {
  ...
  return <section className={`gate4b-feedback-card ${journey.feedback.state}`}><div className="gate4b-card-label"><span>04 · FEEDBACK</span><b>{positive ? "COMPLETED" : "RETRY AVAILABLE"}</b></div><div className="gate4b-feedback-content"><div className="gate4b-feedback-icon">{positive ? <Check size={20} /> : <Target size={19} />}</div><div><span>{journey.feedback.state === "correct" || journey.feedback.state === "valid-alternative" ? "دليل واضح" : journey.feedback.state === "partially-correct" ? "إشارة مفيدة" : "مساحة للمحاولة"}</span><h3>{journey.feedback.title}</h3><p>{journey.feedback.explanation}</p>{journey.feedback.hint && <small><b>تلميح:</b> {journey.feedback.hint}</small>}{journey.feedback.nextStep && <small><b>الخطوة التالية:</b> {journey.feedback.nextStep}</small>}</div><div className="gate4b-score"><strong>{Math.round(journey.assessment.score * 100)}%</strong><span>score</span></div></div><div className="gate4b-feedback-meta"><span>assessment · {shortId(journey.assessment.id)}</span><span>attempt · {shortId(journey.assessment.attemptId)}</span><span>teacher note · {journey.feedback.teacherNote}</span></div></section>;
}
