    ...
    <main dir="rtl" className={`gate4b-shell ${presentation ? "is-presenting" : ""}`}>
      <header className="gate4b-topbar">
        <div className="gate4b-brand">
          ...
          <div><span className="gate4b-eyebrow">مساحة المعلم · شرح وتحقق</span><h1>لوحة تشرح الفكرة وتختبر الفهم</h1></div>
        </div>
      ...
      <section className="gate4b-workspace" aria-label="مساحة التدريس التفاعلية">
        <aside className="gate4b-rail">
          <span className="gate4b-rail-number">04</span><span className="gate4b-rail-line" /><span className="gate4b-rail-label">اشرح · تفاعل · قيّم</span>
          <div className="gate4b-rail-footer"><LockKeyhole size={14} /> محلي</div>
        </aside>

        <section className="gate4b-canvas-column">
          <div className="gate4b-heading-row"><div><span className="gate4b-kicker">مسار تعليمي محدد</span><h2>{presentation ? "اتبع خطوات الفهم" : "من المصدر إلى الدليل"}</h2><p>{presentation ? "اختر، أجب، ثم ناقش التغذية الراجعة." : "سطح واحد؛ أدوات موضوعية تظهر عند الحاجة فقط."}</p></div><div className="gate4b-lesson-stamp"><span>الدرس</span><strong>{shortId(lesson.lessonId)}</strong><small>{restored ? "مستعاد" : "مسودة محلية"}</small></div></div>

          <div className="gate4b-subject-switcher" role="tablist" aria-label="حزم المواد">
            <button className={activeSubject === "arabic" ? "active" : ""} onClick={() => chooseSubject("arabic")} role="tab" aria-selected={activeSubject === "arabic"}><BookOpen size={17} /><span><b>العربية</b><small>التحليل اللغوي · تحديد وتصنيف</small></span><em>A</em></button>
            <button className={activeSubject === "mathematics" ? "active" : ""} onClick={() => chooseSubject("mathematics")} role="tab" aria-selected={activeSubject === "mathematics"}><Gauge size={17} /><span><b>الرياضيات</b><small>التمثيل الرياضي · حل وتحقق</small></span><em>∑</em></button>
          </div>
          ...
          <div className="gate4b-canvas" aria-live="polite">
            <div className="gate4b-canvas-topline"><span><Layers3 size={15} /> لوحة الدرس / {subjectLabel[activeSubject]}</span><span className="gate4b-canvas-note">{objectCapability ? "الكائن يدعم التفاعل" : "كائن عرض"} · {lensReady ? "مرجع المصدر سليم" : "تحقق من المصدر"}</span></div>
            <div className="gate4b-source-card">
              <div className="gate4b-card-label"><span>01 · مصدر المحتوى</span><b>{activeSubject === "arabic" ? "جملة تعليمية" : "معادلة تعليمية"}</b></div>
              <div className={`gate4b-source-content ${activeSubject === "mathematics" ? "math-source" : ""}`}>
                {activeSubject === "arabic" ? <><p className="gate4b-arabic-sentence">{String(journey.source.content)}</p><small>محتوى من إعداد المعلم</small></> : <><p className="gate4b-equation">{String(journey.source.content)}</p><small>مسألة من إعداد المعلم</small></>}
              </div>
            ...
            <div className="gate4b-canvas-footer"><span>{notice}</span><span className="gate4b-provenance-mark"><Check size={13} /> مصدر المحتوى محفوظ</span></div>
          </div>
        </section>

        {!presentation && <aside className="gate4b-inspector" aria-label="لوحة المعلم"><div className="gate4b-inspector-head"><span className="gate4b-kicker">لوحة المعلم</span><h2>لوحة الدليل</h2><p>نفس مسار التفاعل والتقييم والتغذية الراجعة للمادتين.</p></div><div className="gate4b-metric"><span>المصدر</span><strong>{activeSubject === "arabic" ? "جملة تعليمية" : "معادلة تعليمية"}</strong><small>محتوى من إعداد المعلم</small></div><div className="gate4b-metric"><span>التحويل</span><strong>{activeSubject === "arabic" ? "تحليل لغوي" : "تمثيل رياضي"}</strong><small>مبني على المحتوى المحدد</small></div><div className="gate4b-metric"><span>النشاط</span><strong>{journey.activity.completionState === "complete" ? "مكتمل" : "بانتظار الإجابة"}</strong><small>{journey.activity.attemptCount} محاولات</small></div><div className="gate4b-note"><Target size={16} /><p><b>حدود هذه الشريحة</b> تحليل عربي ومعادلة محددان مسبقًا للتأكد من الصحة التعليمية دون أدوات توليدية.</p></div><div className="gate4b-checklist"><span>مؤشرات الدرس</span><p><Check size={13} /> المحتوى محفوظ بصيغته التعليمية</p><p><Check size={13} /> تقييم حتمي قابل للتفسير</p><p><Check size={13} /> حفظ واستعادة متسقان</p></div><button className="gate4b-outline-button" onClick={saveLesson}><FileDown size={15} /> حفظ الحالة الحالية</button><div className="gate4b-shortcut"><Undo2 size={14} /> <span>Ctrl / ⌘ + S<br /><small>اختصار الحفظ</small></span></div></aside>}
      </section>
      <footer className="gate4b-footer"><span>أداة تعليمية عربية أولًا · تعمل من اليمين إلى اليسار</span><span>اللمس والقلم · لم يُتحقق منهما بعد</span><span>مراجعة الدرس متاحة للمعلم</span></footer>
    </main>
  );
}
...
function ArabicLensCard({ journey, presentation, onSelectWord, onReveal, onMode }: { journey: JourneyState & { lens: GrammarLens }; presentation: boolean; onSelectWord: (wordId: string) => void; onReveal: () => void; onMode: (mode: GrammarLens["mode"]) => void }) {
  ...
  return <section className="gate4b-lens-card"><div className="gate4b-card-label"><span>02 · التحليل اللغوي</span><b>نطاق المصدر {journey.lens.sourceRange?.start}–{journey.lens.sourceRange?.end} · المستوى {journey.lens.disclosureLevel}/5</b></div><div className="gate4b-lens-heading"><div><h3>من قام بالفعل؟</h3><p>انقر كلمة لربطها بدورها النحوي.</p></div>{!presentation && <><button className="gate4b-icon-action" onClick={onReveal} aria-label={journey.lens.revealAnswer ? "إخفاء الإجابة" : "الانتقال إلى مستوى الكشف التالي"}>{journey.lens.revealAnswer ? <EyeOff size={16} /> : <Eye size={16} />} {journey.lens.revealAnswer ? "إخفاء" : `المستوى ${journey.lens.disclosureLevel + 1 > 5 ? 1 : journey.lens.disclosureLevel + 1}`}</button><button className="gate4b-quiet-button" onClick={() => onMode(journey.lens.mode === "teacher" ? "student" : "teacher")} aria-pressed={journey.lens.mode === "teacher"}>{journey.lens.mode === "teacher" ? "وضع الطالب" : "وضع المعلم"}</button></>}</div><div className="gate4b-word-row">{journey.lens.words.map((word) => <button key={word.id} className={word.id === journey.lens.selectedWordId ? "selected" : ""} onClick={() => onSelectWord(word.id)} aria-label={`اختيار كلمة ${word.text}`}><span>{word.text}</span>{journey.lens.disclosureLevel >= 2 && <small>{word.grammaticalRole}</small>}</button>)}</div>{selected && <div className="gate4b-selected-word"><b>{selected.text}</b><span>{journey.lens.disclosureLevel >= 3 ? selected.caseMark : journey.lens.disclosureLevel >= 2 ? selected.grammaticalRole : "اضغط كشف الإجابة لرؤية الدور"}</span><small>{journey.lens.disclosureLevel >= 5 ? selected.explanation : journey.lens.disclosureLevel >= 4 ? "اسأل: لماذا تحمل الكلمة هذا الدور؟" : `نطاق الكلمة ${selected.start}–${selected.end}`}</small>{journey.lens.disclosureLevel >= 3 && <div className="gate4b-i3rab-reveal" aria-label="تفاصيل الإعراب"><span><b>الدور</b>{journey.lens.disclosureLevel >= 2 ? selected.grammaticalRole : "مخفي"}</span><span><b>الحالة</b>{journey.lens.disclosureLevel >= 3 ? (selected.grammaticalRole === "مفعول به" ? "منصوب" : "مرفوع") : "مخفي"}</span><span><b>العلامة</b>{journey.lens.disclosureLevel >= 3 ? selected.caseMark.replace("مرفوع · ", "").replace("منصوب · ", "") : "مخفي"}</span><span><b>السبب</b>{journey.lens.disclosureLevel >= 5 ? (selected.explanation.split(" — ")[1] ?? selected.explanation) : "مخفي حتى مستوى لماذا"}</span></div>}</div>}<div className="gate4b-trace-line"><span>مصدر المحتوى</span><strong>{shortId(journey.lens.provenance.sourceObjectId)} ← جملة تعليمية</strong><i>{journey.lens.provenance.teacherApproved ? "معتمد" : "تحليل حتمي من المصدر"}</i></div></section>;
}
...
function MathLensCard({ journey, presentation, onReveal }: { journey: JourneyState & { lens: MathVisualizationLens }; presentation: boolean; onReveal: () => void }) {
  return <section className="gate4b-lens-card math-lens-card"><div className="gate4b-card-label"><span>02 · التمثيل الرياضي</span><b>المعادلة ↔ التمثيل</b></div><div className="gate4b-lens-heading"><div><h3>وازن الطرفين، ثم تحقق</h3><p>تمثيل بصري صغير يثبت نقطة الحل.</p></div>{!presentation && <button className="gate4b-icon-action" onClick={onReveal} aria-label={journey.lens.revealAnswer ? "إخفاء خطوات الحل" : "إظهار خطوات الحل"}>{journey.lens.revealAnswer ? <EyeOff size={16} /> : <Eye size={16} />} {journey.lens.revealAnswer ? "إخفاء" : "كشف الخطوات"}</button>}</div><div className="gate4b-graph"><svg viewBox="0 0 480 132" role="img" aria-label="تمثيل نقطة الحل x يساوي 4"><path d="M34 92H454M76 18V113" /><path className="curve" d="M76 94L150 78L224 62L298 46L372 30L446 14" /><circle cx="372" cy="30" r="6" /><text x="362" y="18">x = 4</text><text x="444" y="111">x</text><text x="48" y="26">y</text></svg><div><span>نقطة التحقق</span><strong>(4, 0)</strong></div></div>{journey.lens.revealAnswer && <div className="gate4b-step-row">{journey.lens.operationSteps.map((step, index) => <div key={step.equation}><span>0{index + 1}</span><b>{step.equation}</b><small>{step.label}</small></div>)}</div>}<div className="gate4b-trace-line"><span>مصدر المحتوى</span><strong>{shortId(journey.lens.provenance.sourceObjectId)} ← معادلة تعليمية</strong><i>تمثيل حتمي من المصدر</i></div></section>;
}
...
function FeedbackCard({ journey, presentation, onOverride }: { journey: JourneyState; presentation: boolean; onOverride: (state: FeedbackState, reason: string, note: string) => void }) {
  ...
  return <section className={`gate4b-feedback-card ${journey.feedback.state}`}><div className="gate4b-card-label"><span>04 · التغذية الراجعة</span><b>{positive ? "اكتمل" : "يمكنك إعادة المحاولة"}</b></div><div className="gate4b-feedback-content"><div className="gate4b-feedback-icon">{positive ? <Check size={20} /> : <Target size={19} />}</div><div><span>{journey.feedback.state === "correct" || journey.feedback.state === "valid-alternative" ? "دليل واضح" : journey.feedback.state === "partially-correct" ? "إشارة مفيدة" : "مساحة للمحاولة"}</span><h3>{journey.feedback.title}</h3><p>{journey.feedback.explanation}</p>{journey.feedback.hint && <small><b>تلميح:</b> {journey.feedback.hint}</small>}{journey.feedback.nextStep && <small><b>الخطوة التالية:</b> {journey.feedback.nextStep}</small>}</div><div className="gate4b-score"><strong>{Math.round((journey.assessment.effectiveEvaluation === "correct" || journey.assessment.effectiveEvaluation === "valid-alternative" ? 1 : journey.assessment.score) * 100)}%</strong><span>نسبة النتيجة</span></div></div><div className="gate4b-feedback-meta"><span>نتيجة التقييم: {statusLabel[journey.assessment.evaluation]}</span><span>النتيجة المعتمدة: {statusLabel[journey.assessment.effectiveEvaluation]}</span><span>التشخيص: {journey.assessment.diagnostic}</span><span>حالة المراجعة: {reviewLabel[journey.assessment.reviewState]}</span></div>{!presentation && <div className="gate4b-override-panel" aria-label="اعتماد المعلم"><div><b>قرار المعلم</b><small>لا يلغي نتيجة التقييم؛ يضيف ملاحظة المعلم إلى سجل الدرس.</small></div><select value={overrideState} onChange={(event) => setOverrideState(event.target.value as FeedbackState)} aria-label="الحالة الفعالة"><option value="correct">صحيحة</option><option value="valid-alternative">بديلة صحيحة</option><option value="partially-correct">جزئية</option><option value="incorrect">غير صحيحة</option><option value="incomplete">غير مكتملة</option></select><input value={reason} onChange={(event) => setReason(event.target.value)} aria-label="سبب قرار المعلم" /><input value={note} onChange={(event) => setNote(event.target.value)} aria-label="ملاحظة قرار المعلم" /><button className="gate4b-quiet-button" onClick={() => onOverride(overrideState, reason, note)}><Check size={14} /> حفظ قرار المعلم</button></div>}</section>;
}
...
function MathStepCard({ session, presentation, onSubmit, onVerify, onOverride }: { session: MathStepSession; presentation: boolean; onSubmit: (step: SolutionStepObject) => void; onVerify: (expression: string) => void; onOverride: (decision: FeedbackState, reason: string) => void }) {
  ...
  return <section className="gate4b-activity-card math-step-card" aria-label="نشاط الحل خطوة بخطوة">
    <div className="gate4b-card-label"><span>03 · الحل خطوة بخطوة</span><b>الخطوة {session.currentStep} من 2</b></div>
    <div className="gate4b-activity-main"><div><span className="gate4b-activity-kicker">المسألة</span><h3>{session.problem.equation}</h3><p>ابنِ الحل خطوة خطوة؛ لا يكفي تطابق الإجابة النهائية.</p></div>{!presentation && <button className="gate4b-quiet-button" onClick={() => setMode(mode === "teacher" ? "student" : "teacher")} aria-pressed={mode === "teacher"}>{mode === "teacher" ? "وضع الطالب" : "وضع المعلم"}</button>}</div>
    <div className="gate4b-i3rab-form math-step-form" aria-label="إجابة الخطوة المنظمة">
      ...
      <label className="gate4b-i3rab-field"><span>العملية</span><input value={operation} onChange={(event) => setOperation(event.target.value)} placeholder="مثال: اطرح 3 من الطرفين" aria-label="العملية الرياضية" /></label>
    ...
    {latest && <div className={`gate4b-selected-word math-step-feedback`}><b>{latest.feedback.title}</b><span>{latest.feedback.explanation}</span>{latest.feedback.hint && session.disclosureLevel >= 2 && <small><b>تلميح:</b> {latest.feedback.hint}</small>}{mode === "teacher" && latest.feedback.correctedStep && <small><b>التصحيح:</b> {latest.feedback.correctedStep.expressionBefore} → {latest.feedback.correctedStep.expressionAfter}</small>}{mode === "teacher" && !presentation && <div className="math-teacher-override" aria-label="اعتماد المعلم للخطوة"><small><b>نتيجة التقييم:</b> {statusLabel[latest.evaluation]} · {latest.diagnostic}</small><select value={teacherDecision} onChange={(event) => setTeacherDecision(event.target.value as FeedbackState)} aria-label="قرار المعلم"><option value="correct">قبول الخطوة</option><option value="valid-alternative">قبول كبديل</option><option value="partially-correct">قبول جزئيًا</option><option value="incorrect">رفض الخطوة</option><option value="incomplete">اعتبارها غير مكتملة</option></select><input value={teacherReason} onChange={(event) => setTeacherReason(event.target.value)} aria-label="سبب قرار المعلم" /><button className="gate4b-quiet-button" onClick={() => onOverride(teacherDecision, teacherReason)}>حفظ قرار المعلم</button><small><b>النتيجة المعتمدة:</b> {statusLabel[latest.effectiveEvaluation]}</small></div>}</div>}
    ...
    <div className="gate4b-trace-line"><span>مصدر المسألة</span><strong>معادلة تعليمية</strong><i>{session.assessments.length} خطوات تم تقييمها</i></div>
  </section>;
}
