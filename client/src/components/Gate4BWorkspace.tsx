...
const statusLabel: Record<FeedbackState, string> = { correct: "صحيحة", "partially-correct": "جزئية", incorrect: "غير صحيحة" };
      ...
      <section className="gate4b-workspace" aria-label="مساحة التدريس التفاعلية">
        ...
        <section className="gate4b-canvas-column">
          ...
          <div className="gate4b-canvas" aria-live="polite">
            ...
            {journey.feedback && <FeedbackCard journey={journey} />}
...
function ActivityCard({ journey, presentation, onAnswer, onSubmit, onRetry }: { journey: JourneyState; presentation: boolean; onAnswer: (value: string) => void; onSubmit: () => void; onRetry: () => void }) {
  ...
  return <section className="gate4b-activity-card"><div className="gate4b-card-label"><span>03 · INTERACTIVE ACTIVITY</span><b>{journey.activity.interactionKind}</b></div><div className="gate4b-activity-main"><div><span className="gate4b-activity-kicker">{arabic ? "CLASSIFY" : "SOLVE"}</span><h3>{journey.activity.prompt}</h3><p>{arabic ? "اختر كلمة ثم أرسل الإجابة." : "أدخل قيمة x ثم تحقق من التكافؤ."}</p></div>{!presentation && <span className="gate4b-attempt-count">{journey.activity.attemptCount} محاولات</span>}</div>{arabic ? <div className="gate4b-answer-pills" role="group" aria-label="اختيار الفاعل">{(journey.lens as GrammarLens).words.map((word) => <button key={word.id} className={journey.activity.answer === word.id ? "chosen" : ""} onClick={() => onAnswer(word.id)}>{word.text}</button>)}</div> : <div className="gate4b-answer-input"><label htmlFor="math-answer">قيمة x</label><input id="math-answer" inputMode="decimal" value={journey.activity.answer} onChange={(event) => onAnswer(event.target.value)} placeholder="مثال: 4" /><span>2x + 3 = 11</span></div>}<div className="gate4b-activity-actions">{!presentation && <button className="gate4b-primary-button" onClick={onSubmit} disabled={!journey.activity.answer}><Check size={15} /> إرسال الإجابة</button>}{journey.assessment && journey.assessment.evaluation !== "correct" && <button className="gate4b-quiet-button" onClick={onRetry}><RotateCcw size={15} /> إعادة المحاولة</button>}<span className="gate4b-assessment-status">{journey.assessment ? `التقييم: ${statusLabel[journey.assessment.evaluation]}` : "لم يُقيّم بعد"}</span></div></section>;
}

function FeedbackCard({ journey }: { journey: JourneyState }) {
  if (!journey.feedback || !journey.assessment) return null;
  const positive = journey.feedback.state === "correct";
  return <section className={`gate4b-feedback-card ${journey.feedback.state}`}><div className="gate4b-card-label"><span>04 · FEEDBACK</span><b>{positive ? "COMPLETED" : "RETRY AVAILABLE"}</b></div><div className="gate4b-feedback-content"><div className="gate4b-feedback-icon">{positive ? <Check size={20} /> : <Target size={19} />}</div><div><span>{journey.feedback.state === "correct" ? "دليل واضح" : journey.feedback.state === "partially-correct" ? "إشارة مفيدة" : "مساحة للمحاولة"}</span><h3>{journey.feedback.title}</h3><p>{journey.feedback.explanation}</p>{journey.feedback.hint && <small><b>تلميح:</b> {journey.feedback.hint}</small>}</div><div className="gate4b-score"><strong>{Math.round(journey.assessment.score * 100)}%</strong><span>score</span></div></div><div className="gate4b-feedback-meta"><span>assessment · {shortId(journey.assessment.id)}</span><span>attempt · {shortId(journey.assessment.attemptId)}</span><span>teacher note · {journey.feedback.teacherNote}</span></div></section>;
}
