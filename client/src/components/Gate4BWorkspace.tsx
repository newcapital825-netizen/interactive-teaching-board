...
function ArabicLensCard({ journey, presentation, onSelectWord, onReveal }: { journey: JourneyState & { lens: GrammarLens }; presentation: boolean; onSelectWord: (wordId: string) => void; onReveal: () => void }) {
  ...
  return <section className="gate4b-lens-card"><div className="gate4b-card-label"><span>02 · GRAMMAR LENS</span><b>sourceRange {journey.lens.sourceRange?.start}–{journey.lens.sourceRange?.end}</b></div><div className="gate4b-lens-heading"><div><h3>من قام بالفعل؟</h3><p>انقر كلمة لربطها بدورها النحوي.</p></div>{!presentation && <button className="gate4b-icon-action" onClick={onReveal} aria-label={journey.lens.revealAnswer ? "إخفاء الإجابة" : "إظهار الإجابة"}>{journey.lens.revealAnswer ? <EyeOff size={16} /> : <Eye size={16} />} {journey.lens.revealAnswer ? "إخفاء" : "كشف الإجابة"}</button>}</div><div className="gate4b-word-row">{journey.lens.words.map((word) => <button key={word.id} className={word.id === journey.lens.selectedWordId ? "selected" : ""} onClick={() => onSelectWord(word.id)} aria-label={`اختيار كلمة ${word.text}`}><span>{word.text}</span>{journey.lens.revealAnswer && <small>{word.grammaticalRole}</small>}</button>)}</div>{selected && <div className="gate4b-selected-word"><b>{selected.text}</b><span>{journey.lens.revealAnswer ? selected.caseMark : "اضغط كشف الإجابة لرؤية الدور"}</span><small>{journey.lens.revealAnswer ? selected.explanation : `word range ${selected.start}–${selected.end}`}</small>{journey.lens.revealAnswer && <div className="gate4b-i3rab-reveal" aria-label="تفاصيل الإعراب"><span><b>الدور</b>{selected.grammaticalRole}</span><span><b>الحالة</b>{selected.grammaticalRole === "مفعول به" ? "منصوب" : "مرفوع"}</span><span><b>العلامة</b>{selected.caseMark.replace("مرفوع · ", "").replace("منصوب · ", "")}</span><span><b>السبب</b>{selected.explanation.split(" — ")[1] ?? selected.explanation}</span></div>}</div>}<div className="gate4b-trace-line"><span>PROVENANCE</span><strong>{shortId(journey.lens.provenance.sourceObjectId)} ← SentenceObject</strong><i>{journey.lens.provenance.teacherApproved ? "approved" : "derived · deterministic"}</i></div></section>;
}
