...
export default function ArabicToolkitPanel() {
  ...
  return <section className="arabic-toolkit-panel" aria-label="أدوات العربية للمعلم"><div className="arabic-toolkit-heading"><div><span className="teacher-product-kicker">مِداد · أدوات العربية</span><h2>من النص إلى نشاط الفهم</h2><p>أدوات محددة المصدر: لا تُنشئ معنى خارج ما كتبه المعلم.</p></div><div className="arabic-toolkit-status"><Check size={15} /> مصدر المحتوى محفوظ</div></div><div className="arabic-toolkit-grid"><div className="arabic-toolkit-source"><label><span>النص أو الفقرة</span><textarea value={state.text} onChange={(event) => update({ text: event.target.value })} aria-label="النص أو الفقرة" /></label><div className="arabic-toolkit-selection"><label><span>النص المحدد</span><input value={state.selection} onChange={(event) => update({ selection: event.target.value })} aria-label="النص المحدد" /></label><span className="arabic-toolkit-range">{selectedIndex >= 0 ? `نطاق المصدر ${selectedIndex}–${selectedIndex + state.selection.length}` : "العبارة غير موجودة في المصدر"}</span></div><label><span>ملاحظة تعليمية</span><input value={state.note} onChange={(event) => update({ note: event.target.value })} aria-label="ملاحظة تعليمية" /></label><div className="arabic-toolkit-actions"><button className="gate4b-primary-button" onClick={addSentenceToBoard}><Save size={14} /> إضافة المصدر للوحة</button><button className="gate4b-quiet-button" onClick={() => setNotice("التمييز محفوظ محليًا كتعليق مع نطاق المصدر.")}><Highlighter size={14} /> تمييز العبارة</button><span><MessageSquarePlus size={14} /> ملاحظة محفوظة</span></div></div><div className="arabic-toolkit-reading"><div className="arabic-toolkit-section-title"><span><BookOpen size={15} /> فهم مقروء</span><small>سؤال من مصدر المعلم</small></div><label><span>السؤال</span><input value={state.prompt} onChange={(event) => update({ prompt: event.target.value })} aria-label="سؤال الفهم" /></label><label><span>الإجابة المقبولة التي يحددها المعلم</span><input value={state.expectedAnswer} onChange={(event) => update({ expectedAnswer: event.target.value })} aria-label="الإجابة المقبولة" /></label><div className="arabic-toolkit-student"><div><span>معاينة الطالب</span><strong>{state.prompt}</strong></div><label><span>إجابة الطالب</span><input value={state.studentAnswer} onChange={(event) => update({ studentAnswer: event.target.value })} aria-label="إجابة الطالب" placeholder="اكتب الإجابة من النص" /></label><button className="gate4b-primary-button" onClick={assessReading}><Play size={14} /> تقييم حتمي</button>{state.result && <div className={`arabic-toolkit-result ${state.result}`}><Target size={15} />{state.result === "correct" ? "صحيحة" : state.result === "partially-correct" ? "جزئية" : "غير صحيحة"}</div>}</div></div></div><p className="arabic-toolkit-notice" role="status">{notice}</p></section>;
}
