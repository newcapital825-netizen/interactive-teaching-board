...
export default function ContextualActionBar({ object, onNotice, onConvert }: Props) {
  ...
  return <section className="contextual-action-bar" aria-label="الإجراءات السياقية"><div className="contextual-action-context"><span>العنصر المحدد</span><strong>{object.type} · {String(object.content).slice(0, 42)}</strong></div><div className="contextual-action-list">{actions.map((action) => { const Icon = icons[action.id]; return <button key={action.id} className={action.available ? "contextual-action available" : "contextual-action unavailable"} aria-disabled={!action.available} title={action.description} onClick={() => perform(action.id, action.available)}><Icon size={14} />{action.label}{action.id === "toggle-visibility" && (selectedObject.metadata.visible ? <Eye size={11} /> : <EyeOff size={11} />)}</button>; })}</div></section>;
}
