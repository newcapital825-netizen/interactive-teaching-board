...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Copy, Group, Maximize2, MousePointer2, Redo2, Trash2, Ungroup, Undo2, ZoomIn, ZoomOut } from "lucide-react";
...
export default function TeacherCanvas({ document, selection, onDocumentChange, onSelectionChange, onNotice }: Props) {
  ...
  const zoomIn = () => { setZoom((value) => Math.min(1.8, Math.round((value + .1) * 100) / 100)); onNotice("تم تكبير اللوحة."); };
  const zoomOut = () => { setZoom((value) => Math.max(.55, Math.round((value - .1) * 100) / 100)); onNotice("تم تصغير اللوحة."); };
  const fitContent = () => { setZoom(1); setPan({ x: 0, y: 0 }); onNotice("أُعيدت اللوحة إلى الحجم والموضع الأصليين."); };
  ...
  const keyboard = (event: KeyboardEvent) => {
    ...
    if (!command) return;
    const actions: Record<string, () => void> = { undo, redo, copy, paste, duplicate, delete: remove, selectAll: () => onSelectionChange({ ids: page.objects.map((object) => object.id), primaryId: page.objects.at(-1)?.id ?? null }), moveLeft: () => updateObjects(moveObjects(page.objects, safeSelection.ids, -12, 0), "تم تحريك الاختيار."), moveRight: () => updateObjects(moveObjects(page.objects, safeSelection.ids, 12, 0), "تم تحريك الاختيار."), moveUp: () => updateObjects(moveObjects(page.objects, safeSelection.ids, 0, -12), "تم تحريك الاختيار."), moveDown: () => updateObjects(moveObjects(page.objects, safeSelection.ids, 0, 12), "تم تحريك الاختيار."), zoomIn, zoomOut, fitContent };
  ...
  return <section className="core-board ux-board gate11-canvas" aria-label="لوحة التفاعل المباشر" data-testid="teacher-canvas"><div className="core-board-header ux-header"><div><span className="kicker">مساحة الدرس</span><h2>لوحة الدرس</h2><p>اكتب، حرّك، ثم اختر الإجراء المناسب من قرب المحتوى.</p></div><div className="header-status"><span className="core-notice">{safeSelection.ids.length ? `محدد: ${safeSelection.ids.length}` : "لا تحديد"}</span></div></div><div className="core-toolbar ux-toolbar"><div className="toolbar-section primary-tools"><button className={`tool ${tool === "select" ? "active" : ""}`} onClick={() => setTool("select")} aria-label="تحديد"><MousePointer2 size={16} /></button><button className="tool" onClick={() => addObject("TextObject")} aria-label="إضافة نص">T</button><button className="tool" onClick={() => addObject("SentenceObject")} aria-label="إضافة جملة">ج</button><button className="tool" onClick={() => addObject("EquationObject")} aria-label="إضافة معادلة">∑</button><button className="tool" onClick={() => addObject("ShapeObject")} aria-label="إضافة شكل">□</button></div><div className="toolbar-section utility-tools"><button className="tool" onClick={undo} aria-label="تراجع"><Undo2 size={16} /></button><button className="tool" onClick={redo} aria-label="إعادة"><Redo2 size={16} /></button><button className="tool" onClick={zoomIn} aria-label="تكبير"><ZoomIn size={16} /></button><button className="tool" onClick={zoomOut} aria-label="تصغير"><ZoomOut size={16} /></button><button className="tool" onClick={fitContent} aria-label="إعادة الحجم الأصلي" disabled={zoom === 1 && pan.x === 0 && pan.y === 0}><Maximize2 size={16} /></button></div></div>{selected && <div className="context-strip" role="toolbar" aria-label="إجراءات العنصر المحدد"><strong>{labelFor(selected.type)}</strong><button onClick={duplicate}><Copy size={14} /> تكرار</button><button onClick={remove}><Trash2 size={14} /> حذف</button>{selected.type === "GroupObject" ? <button onClick={ungroup}><Ungroup size={14} /> فك</button> : <button onClick={group} disabled={selectedObjects.length < 2}><Group size={14} /> تجميع</button>}</div>}<div className="core-layout ux-layout"><div className="ux-stage-wrap"><div className="stage-meta"><span>{page.name} · {document.title}</span><span>{Math.round(zoom * 100)}% · {page.objects.length} عناصر</span></div><div ref={stageRef} className="core-stage ux-stage" data-testid="canvas-stage" onPointerDown={() => onSelectionChange(emptySelection())} onPointerMove={move} onPointerUp={finishDrag} onPointerCancel={finishDrag} onWheel={(event) => { event.preventDefault(); if (event.ctrlKey || event.metaKey) { const direction = event.deltaY > 0 ? -1 : 1; setZoom((value) => Math.min(1.8, Math.max(.55, Math.round((value + direction * .08) * 100) / 100))); return; } setPan((value) => ({ x: value.x - event.deltaX, y: value.y - event.deltaY })); }} aria-label="سطح اللوحة المباشر"><div className="canvas-viewport" data-testid="canvas-viewport" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}><div className="core-grid" />{[...page.objects].sort((a, b) => a.zIndex - b.zIndex).map((item) => item.metadata.visible && <button key={item.id} className={`core-object ux-object ${safeSelection.ids.includes(item.id) ? "selected" : ""} ${item.metadata.locked ? "locked" : ""}`} data-testid="canvas-object" style={{ right: item.position.x, top: item.position.y, width: item.size.width, height: item.size.height, transform: `rotate(${item.rotation}deg)`, color: item.style.color, background: item.style.background }} onPointerDown={(event) => select(event, item.id)} onKeyDown={(event) => selectKeyboard(event, item.id)} aria-pressed={safeSelection.ids.includes(item.id)} aria-label={`${labelFor(item.type)}: ${contentFor(item)}`}><span className="core-object-type">{labelFor(item.type)}</span><strong>{contentFor(item)}</strong><small>{safeSelection.ids.includes(item.id) ? "محدد مباشرة" : "جاهز للشرح"}</small>{safeSelection.primaryId === item.id && <><span className="resize-handle handle-tl" onPointerDown={(event) => beginResize(event, item, "tl")} aria-hidden="true" /><span className="resize-handle handle-tr" onPointerDown={(event) => beginResize(event, item, "tr")} aria-hidden="true" /><span className="resize-handle handle-bl" onPointerDown={(event) => beginResize(event, item, "bl")} aria-hidden="true" /><span className="resize-handle handle-br" onPointerDown={(event) => beginResize(event, item, "br")} aria-hidden="true" /></>}</button>)}</div></div></div><aside className="object-inspector ux-inspector"><div className="panel-title"><span><small className="panel-kicker">العنصر المحدد</small>تفاصيل التحديد</span><span className="inspector-status">{safeSelection.ids.length}</span></div>{selected ? <div className="inspector-content"><label>المحتوى<input data-text-editor="true" value={selected.content} onChange={editContent} aria-label="تحرير محتوى العنصر المحدد" /></label><div className="inspector-grid"><button onClick={() => updateObjects(moveObjects(page.objects, safeSelection.ids, -12, 0), "تم تحريك العنصر.")}><ArrowLeft size={13} /> تحريك</button><button onClick={() => updateObjects(moveObjects(page.objects, safeSelection.ids, 12, 0), "تم تحريك العنصر.")}>تحريك <ArrowRight size={13} /></button><button onClick={() => layer("front")}>إلى الأمام</button><button onClick={() => layer("back")}>إلى الخلف</button></div><div className="inspector-grid accessible-resize-controls"><button onClick={() => resizeByKeyboard(12, 0)} aria-label="زيادة عرض العنصر">زيادة العرض</button><button onClick={() => resizeByKeyboard(-12, 0)} aria-label="تقليل عرض العنصر">تقليل العرض</button><button onClick={() => resizeByKeyboard(0, 12)} aria-label="زيادة ارتفاع العنصر">زيادة الارتفاع</button><button onClick={() => resizeByKeyboard(0, -12)} aria-label="تقليل ارتفاع العنصر">تقليل الارتفاع</button></div><div className="inspector-grid"><button onClick={() => align("left")}>محاذاة يسار</button><button onClick={() => align("center")}>توسيط</button><button onClick={() => align("top")}>محاذاة أعلى</button><button onClick={() => distribute("horizontal")}>توزيع أفقي</button></div><p className="teacher-product-notice">{selected.source ? "هذا المحتوى مرتبط بالمصدر الأصلي للدرس." : "هذا المحتوى أضيف مباشرة إلى الدرس."}</p></div> : <div className="empty-inspector">حدد عنصرًا مباشرة من اللوحة لرؤية أدواته.</div>}</aside></div></section>;
}
