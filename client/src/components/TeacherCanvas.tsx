...
export default function TeacherCanvas({ document, selection, onDocumentChange, onSelectionChange, onNotice }: Props) {
  ...
  return <section className="core-board ux-board gate11-canvas" aria-label="لوحة التفاعل المباشر"><div className="core-board-header ux-header"><div><span className="kicker">GATE 11 · DIRECT CANVAS</span><h2>اللوحة التي تتعامل معها مباشرة</h2><p>حدد العنصر على سطح اللوحة، ثم حرّكه أو غيّر حجمه أو افتح إجراءاته.</p></div><div className="header-status"><span className="core-notice">{safeSelection.ids.length ? `محدد: ${safeSelection.ids.length}` : "لا تحديد"}</span></div></div><div className="core-toolbar ux-toolbar"><div className="toolbar-section primary-tools"><button className={`tool ${tool === "select" ? "active" : ""}`} onClick={() => setTool("select")} aria-label="تحديد"><MousePointer2 size={16} /></button><button className="tool" onClick={() => addObject("TextObject")} aria-label="إضافة نص">T</button><button className="tool" onClick={() => addObject("SentenceObject")} aria-label="إضافة جملة">ج</button><button className="tool" onClick={() => addObject("EquationObject")} aria-label="إضافة معادلة">∑</button><button className="tool" onClick={() => addObject("ShapeObject")} aria-label="إضافة شكل">□</button></div><div className="toolbar-section utility-tools"><button className="tool" onClick={undo} aria-label="تراجع"><Undo2 size={16} /></button><button className="tool" onClick={redo} aria-label="إعادة"><Redo2 size={16} /></button><button className="tool" onClick={() => setZoom((value) => Math.min(1.8, value + .1))} aria-label="تكبير"><ZoomIn size={16} /></button><button className="tool" onClick={() => setZoom((value) => Math.max(.55, value - .1))} aria-label="تصغير"><ZoomOut size={16} /></button></div></div>{selected && <div className="context-strip" role="toolbar" aria-label="إجراءات العنصر المحدد"><strong>{labelFor(selected.type)}</strong><button onClick={duplicate}><Copy size={14} /> تكرار</button><button onClick={remove}><Trash2 size={14} /> حذف</button>{selected.type === "GroupObject" ? <button onClick={ungroup}><Ungroup size={14} /> فك</button> : <button onClick={group} disabled={selectedObjects.length < 2}><Group size={14} /> تجميع</button>}</div>}<div className="core-layout ux-layout"><div className="ux-stage-wrap"><div className="stage-meta"><span>{page.name} · {document.title}</span><span>{Math.round(zoom * 100)}% · {page.objects.length} عناصر</span></div><div ref={stageRef} className="core-stage ux-stage" onPointerDown={() => onSelectionChange(emptySelection())} onPointerMove={move} onPointerUp={finishDrag} onPointerCancel={finishDrag} onWheel={(event) => { event.preventDefault(); setPan((value) => ({ x: value.x - event.deltaX, y: value.y - event.deltaY })); }} aria-label="سطح اللوحة المباشر"><div className="core-grid" />{[...page.objects].sort((a, b) => a.zIndex - b.zIndex).map((item) => item.metadata.visible && <button key={item.id} className={`core-object ux-object ${safeSelection.ids.includes(item.id) ? "selected" : ""} ${item.metadata.locked ? "locked" : ""}`} style={{ right: item.position.x, top: item.position.y, width: item.size.width, height: item.size.height, transform: `rotate(${item.rotation}deg)`, color: item.style.color, background: item.style.background }} onPointerDown={(event) => select(event, item.id)} aria-label={`${labelFor(item.type)}: ${contentFor(item)}`}><span className="core-object-type">{labelFor(item.type)}</span><strong>{contentFor(item)}</strong><small>{safeSelection.ids.includes(item.id) ? "محدد مباشرة" : "جاهز للشرح"}</small>{safeSelection.primaryId === item.id && <><span className="resize-handle handle-tl" onPointerDown={(event) => beginResize(event, item, "tl")} role="button" aria-label="تغيير الحجم من أعلى اليسار" /><span className="resize-handle handle-tr" onPointerDown={(event) => beginResize(event, item, "tr")} role="button" aria-label="تغيير الحجم من أعلى اليمين" /><span className="resize-handle handle-bl" onPointerDown={(event) => beginResize(event, item, "bl")} role="button" aria-label="تغيير الحجم من أسفل اليسار" /><span className="resize-handle handle-br" onPointerDown={(event) => beginResize(event, item, "br")} role="button" aria-label="تغيير الحجم من أسفل اليمين" /></>}</button>)}</div></div><aside className="object-inspector ux-inspector"><div className="panel-title"><span><small className="panel-kicker">CANONICAL OBJECT</small>تفاصيل التحديد</span><span className="inspector-status">{safeSelection.ids.length}</span></div>{selected ? <div className="inspector-content"><label>المحتوى<input data-text-editor="true" value={selected.content} onChange={editContent} aria-label="تحرير محتوى العنصر المحدد" /></label><div className="inspector-grid"><button onClick={() => updateObjects(moveObjects(page.objects, safeSelection.ids, -12, 0), "تم تحريك العنصر.")}><ArrowLeft size={13} /> تحريك</button><button onClick={() => updateObjects(moveObjects(page.objects, safeSelection.ids, 12, 0), "تم تحريك العنصر.")}>تحريك <ArrowRight size={13} /></button><button onClick={() => layer("front")}>إلى الأمام</button><button onClick={() => layer("back")}>إلى الخلف</button></div><div className="inspector-grid"><button onClick={() => align("left")}>محاذاة يسار</button><button onClick={() => align("center")}>توسيط</button><button onClick={() => align("top")}>محاذاة أعلى</button><button onClick={() => distribute("horizontal")}>توزيع أفقي</button></div><p className="teacher-product-notice">ID: {selected.id} · {selected.source ? `sourceObjectId محفوظ: ${selected.source.reference ?? "نعم"}` : "مصدر المعلم"}</p></div> : <div className="empty-inspector">حدد عنصرًا مباشرة من اللوحة لرؤية أدواته.</div>}</aside></div></section>;
}
